...
describe("FillingRoleSelector", () => {
  ...
  it("should render user avatar correctly", () => {
    ...
    expect(avatars[0]).toHaveAttribute("class", "avatar");
  });

  it("should render add role button when no user assigned", () => {
    const propsWithoutUsers = {
      ...defaultProps,
      users: [],
    };
    render(<FillingRoleSelector {...propsWithoutUsers} />);

    const addButton = screen.getByTestId("add-role-button");
    expect(addButton).toBeInTheDocument();
  });

  it("should call onAddUser when add button is clicked", () => {
    const propsWithoutUsers = {
      ...defaultProps,
      users: [],
    };
  ...
  it("should call onRemoveUser when remove button is clicked", () => {
    ...
    expect(mockOnRemoveUser).toHaveBeenCalled();
  });
});
