...
import { TInfoPanelMemberType } from "./Members.types";
...
export const useMembers = ({ room, scrollToTop }: UseMembersProps) => {
    ...
    (
      ...
      const membersList: {
      ...
      }[] = [
        {
          ...
          label: t("Common:RoomAdministration"),
          list: administrators,
        },
        {
        ...
        {
          key: TInfoPanelMemberType.guests,
          label: t("Common:RoomExpectUsers"),
  ...
  const fetchMembers = React.useCallback(async () => {
    ...
    try {
      ...
      if (links) {
        const linkItems = links as unknown as TFileLink[];
        setPrimaryLink(linkItems.find((l) => l.sharedTo.primary) ?? null);
        setAdditionalLinks(linkItems.filter((l) => !l.sharedTo.primary));
      } else {
        setPrimaryLink(null);
        setAdditionalLinks([]);
      }
