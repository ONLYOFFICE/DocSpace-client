...
// SDK port of client ChangeRoomOwner. Reads current user via AgentsUserStore
// instead of MobX userStore.user. `currentColorScheme` is dropped — the link
// uses the default accent.
  ...
  return (
    <div>
      <Text className="change-owner-label" fontWeight={600} fontSize="13px">
        {isAgent
          ? t("Common:AgentOwner", { defaultValue: "Agent owner" })
          : t("Common:RoomOwner", { defaultValue: "Room owner" })}
      </Text>
      ...
      <div className={styles.changeOwnerDisplayWrapper}>
        ...
        {canChangeOwner ? (
          ...
          >
            {t("Common:ChangeButton", { defaultValue: "Change" })}
          </Link>
        ) : null}
      </div>
    </div>
...
export default observer(ChangeRoomOwner);
