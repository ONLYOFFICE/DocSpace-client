...
// connection" bug: dropped Next.js chunk requests must be retried by the
// inline bootstrap from @docspace/shared (see the doceditor twin spec for
// the reference scenario). Failures are reproduced with route
// interception; visibility/online gating details are unit tested in
// @docspace/shared.
const PAGE_PATH = "/management/settings";
const CHUNKS_ROUTE = "**/_next/static/chunks/**";

const isFrameworkBooted = () =>
  Boolean((window as { next?: { version?: string } }).next?.version);

test.describe("Chunk load recovery", () => {
  test.beforeEach(({ serverRequestInterceptor, port }) => {
  ...
  test("ships the retry bootstrap in the SSR head", async ({
    page,
    baseUrl,
  }) => {
  ...
  test("recovers when every chunk request fails once", async ({
    ...
    await expect
      .poll(() => page.evaluate(isFrameworkBooted).catch(() => false), {
        timeout: 30_000,
      })
      .toBe(true);
  ...
  test("recovers after a network outage while the page is loading", async ({
    page,
    baseUrl,
  }) => {
    let outage = true;
