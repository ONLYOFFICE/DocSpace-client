...
const DeletePortalDialog = () => {
    ...
    const {
      ...
      const onDelete = () => {
       ...
       logout().then(() => window.open(`${protocol}//${domain}`, "_self"));
    }
    ...
    return (
        ...
        >
            ...
            <Trans i18nKey="DeletePortalText" t={t} domain={domain} displayName={displayName} email={email}>  
                Please note: only the owner is able to delete the selected DocSpace. The owner of <strong>{{domain}}</strong> is <strong>{{displayName}}</strong> <Link className="email-link" type="page" href={`mailto:${email}`} noHover color={currentColorScheme?.main?.accent} title={email}>{{ email }} </Link>. If you are not the owner, you will not be able to access the DocSpace deletion settings by clicking the DELETE button and will be redirected to the Rooms section.   
            </Trans>
            </ModalDialog.Body>
            <ModalDialog.Footer>
        <Button
