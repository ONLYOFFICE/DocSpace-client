...
const ModelSettings = ({ agentParams, setAgentParams }: ModelSettingsProps) => {
  ...
  const systemAiEnabled = aiConfigStore.aiConfig?.systemAiEnabled;
  ...
  const providerOptions = React.useMemo<TOption[]>(
    () =>
  ...
  const providerSelectedOption = React.useMemo<TOption>(
    () =>
      selectedProvider && selectedProvider.id !== -2
        ? {
  ...
  const modelSelectedOption = React.useMemo<TOption>(
    () =>
  ...
  React.useEffect(() => {
    ...
    if (!selectedModel && error) {
      setAgentParams({ modelId: undefined });
      ...
      <div className=" set_room_params-info">
        ...
        !isModelsLoading ? (
          ...
          <ComboBox
            ...
            selectedOption={modelSelectedOption}
...
export default observer(ModelSettings);
