...
const getInitial = (text: any) => text.substring(0, 1).toUpperCase();
...
const CatalogItem = (props: any) => {
  ...
  const onClickBadgeAction = (e: any) => {
    e.stopPropagation();
  ...
  const renderItem = () => {
    return (
      // @ts-expect-error TS(2322): Type '{ children: any[]; className: any; style: an... Remove this comment to see the full error message
      <ColorTheme
        className={className}
        style={style}
        showText={showText}
