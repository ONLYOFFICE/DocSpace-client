...
const DeleteDialogComponent = ({
  ...
  const { deleteAIProvider } = useAISettingsStore();
  const { fetchAIConfig: getAIConfig } = useAgentsAIConfigStore();
  ...
  const onSubmit = async () => {
    try {
      ...
      toastr.success(
        t("Common:ProviderRemovedSuccess", {
  ...
  const bodyText = showDefaultProviderWarning ? (
    <Trans
      t={t}
      i18nKey="Common:DeleteDefaultProviderDescription"
    ...
    t("Common:DeleteProviderDescription")
  );
...
export const DeleteAIProviderDialog = observer(DeleteDialogComponent);
