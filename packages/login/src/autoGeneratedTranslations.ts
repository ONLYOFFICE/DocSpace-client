//THIS FILE IS AUTO GENERATED
//DO NOT EDIT AND DELETE IT

import ErrorsarSAUrl from "ASSETS_DIR/locales/ar-SA/Errors.json?url";

import LoginarSAUrl from "ASSETS_DIR/locales/ar-SA/Login.json?url";

import ErrorsazUrl from "ASSETS_DIR/locales/az/Errors.json?url";

import LoginazUrl from "ASSETS_DIR/locales/az/Login.json?url";

import ErrorsbgUrl from "ASSETS_DIR/locales/bg/Errors.json?url";

import LoginbgUrl from "ASSETS_DIR/locales/bg/Login.json?url";

import ErrorscsUrl from "ASSETS_DIR/locales/cs/Errors.json?url";

import LogincsUrl from "ASSETS_DIR/locales/cs/Login.json?url";

import ErrorsdeUrl from "ASSETS_DIR/locales/de/Errors.json?url";

import LogindeUrl from "ASSETS_DIR/locales/de/Login.json?url";

import ErrorselGRUrl from "ASSETS_DIR/locales/el-GR/Errors.json?url";

import LoginelGRUrl from "ASSETS_DIR/locales/el-GR/Login.json?url";

import ErrorsenUrl from "ASSETS_DIR/locales/en/Errors.json?url";

import LoginenUrl from "ASSETS_DIR/locales/en/Login.json?url";

import ErrorsesUrl from "ASSETS_DIR/locales/es/Errors.json?url";

import LoginesUrl from "ASSETS_DIR/locales/es/Login.json?url";

import ErrorsfiUrl from "ASSETS_DIR/locales/fi/Errors.json?url";

import LoginfiUrl from "ASSETS_DIR/locales/fi/Login.json?url";

import ErrorsfrUrl from "ASSETS_DIR/locales/fr/Errors.json?url";

import LoginfrUrl from "ASSETS_DIR/locales/fr/Login.json?url";

import ErrorshyAMUrl from "ASSETS_DIR/locales/hy-AM/Errors.json?url";

import LoginhyAMUrl from "ASSETS_DIR/locales/hy-AM/Login.json?url";

import ErrorsitUrl from "ASSETS_DIR/locales/it/Errors.json?url";

import LoginitUrl from "ASSETS_DIR/locales/it/Login.json?url";

import ErrorsjaJPUrl from "ASSETS_DIR/locales/ja-JP/Errors.json?url";

import LoginjaJPUrl from "ASSETS_DIR/locales/ja-JP/Login.json?url";

import ErrorskoKRUrl from "ASSETS_DIR/locales/ko-KR/Errors.json?url";

import LoginkoKRUrl from "ASSETS_DIR/locales/ko-KR/Login.json?url";

import ErrorsloLAUrl from "ASSETS_DIR/locales/lo-LA/Errors.json?url";

import LoginloLAUrl from "ASSETS_DIR/locales/lo-LA/Login.json?url";

import ErrorslvUrl from "ASSETS_DIR/locales/lv/Errors.json?url";

import LoginlvUrl from "ASSETS_DIR/locales/lv/Login.json?url";

import ErrorsnlUrl from "ASSETS_DIR/locales/nl/Errors.json?url";

import LoginnlUrl from "ASSETS_DIR/locales/nl/Login.json?url";

import ErrorsplUrl from "ASSETS_DIR/locales/pl/Errors.json?url";

import LoginplUrl from "ASSETS_DIR/locales/pl/Login.json?url";

import ErrorsptUrl from "ASSETS_DIR/locales/pt/Errors.json?url";

import LoginptUrl from "ASSETS_DIR/locales/pt/Login.json?url";

import ErrorsptBRUrl from "ASSETS_DIR/locales/pt-BR/Errors.json?url";

import LoginptBRUrl from "ASSETS_DIR/locales/pt-BR/Login.json?url";

import ErrorsroUrl from "ASSETS_DIR/locales/ro/Errors.json?url";

import LoginroUrl from "ASSETS_DIR/locales/ro/Login.json?url";

import ErrorsruUrl from "ASSETS_DIR/locales/ru/Errors.json?url";

import LoginruUrl from "ASSETS_DIR/locales/ru/Login.json?url";

import ErrorssiUrl from "ASSETS_DIR/locales/si/Errors.json?url";

import LoginsiUrl from "ASSETS_DIR/locales/si/Login.json?url";

import ErrorsskUrl from "ASSETS_DIR/locales/sk/Errors.json?url";

import LoginskUrl from "ASSETS_DIR/locales/sk/Login.json?url";

import ErrorsslUrl from "ASSETS_DIR/locales/sl/Errors.json?url";

import LoginslUrl from "ASSETS_DIR/locales/sl/Login.json?url";

import ErrorssrLatnRSUrl from "ASSETS_DIR/locales/sr-Latn-RS/Errors.json?url";

import LoginsrLatnRSUrl from "ASSETS_DIR/locales/sr-Latn-RS/Login.json?url";

import ErrorstrUrl from "ASSETS_DIR/locales/tr/Errors.json?url";

import LogintrUrl from "ASSETS_DIR/locales/tr/Login.json?url";

import ErrorsukUAUrl from "ASSETS_DIR/locales/uk-UA/Errors.json?url";

import LoginukUAUrl from "ASSETS_DIR/locales/uk-UA/Login.json?url";

import ErrorsviUrl from "ASSETS_DIR/locales/vi/Errors.json?url";

import LoginviUrl from "ASSETS_DIR/locales/vi/Login.json?url";

import ErrorszhCNUrl from "ASSETS_DIR/locales/zh-CN/Errors.json?url";

import LoginzhCNUrl from "ASSETS_DIR/locales/zh-CN/Login.json?url";

import CommonarSAUrl from "PUBLIC_DIR/locales/ar-SA/Common.json?url";

import CommonazUrl from "PUBLIC_DIR/locales/az/Common.json?url";

import CommonbgUrl from "PUBLIC_DIR/locales/bg/Common.json?url";

import CommoncsUrl from "PUBLIC_DIR/locales/cs/Common.json?url";

import CommondeUrl from "PUBLIC_DIR/locales/de/Common.json?url";

import CommonelGRUrl from "PUBLIC_DIR/locales/el-GR/Common.json?url";

import CommonenUrl from "PUBLIC_DIR/locales/en/Common.json?url";

import CommonesUrl from "PUBLIC_DIR/locales/es/Common.json?url";

import CommonfiUrl from "PUBLIC_DIR/locales/fi/Common.json?url";

import CommonfrUrl from "PUBLIC_DIR/locales/fr/Common.json?url";

import CommonhyAMUrl from "PUBLIC_DIR/locales/hy-AM/Common.json?url";

import CommonitUrl from "PUBLIC_DIR/locales/it/Common.json?url";

import CommonjaJPUrl from "PUBLIC_DIR/locales/ja-JP/Common.json?url";

import CommonkoKRUrl from "PUBLIC_DIR/locales/ko-KR/Common.json?url";

import CommonloLAUrl from "PUBLIC_DIR/locales/lo-LA/Common.json?url";

import CommonlvUrl from "PUBLIC_DIR/locales/lv/Common.json?url";

import CommonnlUrl from "PUBLIC_DIR/locales/nl/Common.json?url";

import CommonplUrl from "PUBLIC_DIR/locales/pl/Common.json?url";

import CommonptUrl from "PUBLIC_DIR/locales/pt/Common.json?url";

import CommonptBRUrl from "PUBLIC_DIR/locales/pt-BR/Common.json?url";

import CommonroUrl from "PUBLIC_DIR/locales/ro/Common.json?url";

import CommonruUrl from "PUBLIC_DIR/locales/ru/Common.json?url";

import CommonsiUrl from "PUBLIC_DIR/locales/si/Common.json?url";

import CommonskUrl from "PUBLIC_DIR/locales/sk/Common.json?url";

import CommonslUrl from "PUBLIC_DIR/locales/sl/Common.json?url";

import CommonsrLatnRSUrl from "PUBLIC_DIR/locales/sr-Latn-RS/Common.json?url";

import CommontrUrl from "PUBLIC_DIR/locales/tr/Common.json?url";

import CommonukUAUrl from "PUBLIC_DIR/locales/uk-UA/Common.json?url";

import CommonviUrl from "PUBLIC_DIR/locales/vi/Common.json?url";

import CommonzhCNUrl from "PUBLIC_DIR/locales/zh-CN/Common.json?url";

import FilesarSAUrl from "CLIENT_PUBLIC_DIR/locales/ar-SA/Files.json?url";

import FilesazUrl from "CLIENT_PUBLIC_DIR/locales/az/Files.json?url";

import FilesbgUrl from "CLIENT_PUBLIC_DIR/locales/bg/Files.json?url";

import FilescsUrl from "CLIENT_PUBLIC_DIR/locales/cs/Files.json?url";

import FilesdeUrl from "CLIENT_PUBLIC_DIR/locales/de/Files.json?url";

import FileselGRUrl from "CLIENT_PUBLIC_DIR/locales/el-GR/Files.json?url";

import FilesenUrl from "CLIENT_PUBLIC_DIR/locales/en/Files.json?url";

import FilesesUrl from "CLIENT_PUBLIC_DIR/locales/es/Files.json?url";

import FilesfiUrl from "CLIENT_PUBLIC_DIR/locales/fi/Files.json?url";

import FilesfrUrl from "CLIENT_PUBLIC_DIR/locales/fr/Files.json?url";

import FileshyAMUrl from "CLIENT_PUBLIC_DIR/locales/hy-AM/Files.json?url";

import FilesitUrl from "CLIENT_PUBLIC_DIR/locales/it/Files.json?url";

import FilesjaJPUrl from "CLIENT_PUBLIC_DIR/locales/ja-JP/Files.json?url";

import FileskoKRUrl from "CLIENT_PUBLIC_DIR/locales/ko-KR/Files.json?url";

import FilesloLAUrl from "CLIENT_PUBLIC_DIR/locales/lo-LA/Files.json?url";

import FileslvUrl from "CLIENT_PUBLIC_DIR/locales/lv/Files.json?url";

import FilesnlUrl from "CLIENT_PUBLIC_DIR/locales/nl/Files.json?url";

import FilesplUrl from "CLIENT_PUBLIC_DIR/locales/pl/Files.json?url";

import FilesptUrl from "CLIENT_PUBLIC_DIR/locales/pt/Files.json?url";

import FilesptBRUrl from "CLIENT_PUBLIC_DIR/locales/pt-BR/Files.json?url";

import FilesroUrl from "CLIENT_PUBLIC_DIR/locales/ro/Files.json?url";

import FilesruUrl from "CLIENT_PUBLIC_DIR/locales/ru/Files.json?url";

import FilessiUrl from "CLIENT_PUBLIC_DIR/locales/si/Files.json?url";

import FilesskUrl from "CLIENT_PUBLIC_DIR/locales/sk/Files.json?url";

import FilesslUrl from "CLIENT_PUBLIC_DIR/locales/sl/Files.json?url";

import FilessrLatnRSUrl from "CLIENT_PUBLIC_DIR/locales/sr-Latn-RS/Files.json?url";

import FilestrUrl from "CLIENT_PUBLIC_DIR/locales/tr/Files.json?url";

import FilesukUAUrl from "CLIENT_PUBLIC_DIR/locales/uk-UA/Files.json?url";

import FilesviUrl from "CLIENT_PUBLIC_DIR/locales/vi/Files.json?url";

import FileszhCNUrl from "CLIENT_PUBLIC_DIR/locales/zh-CN/Files.json?url";

const arSA = new Map([
  ["Errors", ErrorsarSAUrl],
  ["Login", LoginarSAUrl],
  ["Common", CommonarSAUrl],
  ["Files", FilesarSAUrl],
]);
const az = new Map([
  ["Errors", ErrorsazUrl],
  ["Login", LoginazUrl],
  ["Common", CommonazUrl],
  ["Files", FilesazUrl],
]);
const bg = new Map([
  ["Errors", ErrorsbgUrl],
  ["Login", LoginbgUrl],
  ["Common", CommonbgUrl],
  ["Files", FilesbgUrl],
]);
const cs = new Map([
  ["Errors", ErrorscsUrl],
  ["Login", LogincsUrl],
  ["Common", CommoncsUrl],
  ["Files", FilescsUrl],
]);
const de = new Map([
  ["Errors", ErrorsdeUrl],
  ["Login", LogindeUrl],
  ["Common", CommondeUrl],
  ["Files", FilesdeUrl],
]);
const elGR = new Map([
  ["Errors", ErrorselGRUrl],
  ["Login", LoginelGRUrl],
  ["Common", CommonelGRUrl],
  ["Files", FileselGRUrl],
]);
const en = new Map([
  ["Errors", ErrorsenUrl],
  ["Login", LoginenUrl],
  ["Common", CommonenUrl],
  ["Files", FilesenUrl],
]);
const es = new Map([
  ["Errors", ErrorsesUrl],
  ["Login", LoginesUrl],
  ["Common", CommonesUrl],
  ["Files", FilesesUrl],
]);
const fi = new Map([
  ["Errors", ErrorsfiUrl],
  ["Login", LoginfiUrl],
  ["Common", CommonfiUrl],
  ["Files", FilesfiUrl],
]);
const fr = new Map([
  ["Errors", ErrorsfrUrl],
  ["Login", LoginfrUrl],
  ["Common", CommonfrUrl],
  ["Files", FilesfrUrl],
]);
const hyAM = new Map([
  ["Errors", ErrorshyAMUrl],
  ["Login", LoginhyAMUrl],
  ["Common", CommonhyAMUrl],
  ["Files", FileshyAMUrl],
]);
const it = new Map([
  ["Errors", ErrorsitUrl],
  ["Login", LoginitUrl],
  ["Common", CommonitUrl],
  ["Files", FilesitUrl],
]);
const jaJP = new Map([
  ["Errors", ErrorsjaJPUrl],
  ["Login", LoginjaJPUrl],
  ["Common", CommonjaJPUrl],
  ["Files", FilesjaJPUrl],
]);
const koKR = new Map([
  ["Errors", ErrorskoKRUrl],
  ["Login", LoginkoKRUrl],
  ["Common", CommonkoKRUrl],
  ["Files", FileskoKRUrl],
]);
const loLA = new Map([
  ["Errors", ErrorsloLAUrl],
  ["Login", LoginloLAUrl],
  ["Common", CommonloLAUrl],
  ["Files", FilesloLAUrl],
]);
const lv = new Map([
  ["Errors", ErrorslvUrl],
  ["Login", LoginlvUrl],
  ["Common", CommonlvUrl],
  ["Files", FileslvUrl],
]);
const nl = new Map([
  ["Errors", ErrorsnlUrl],
  ["Login", LoginnlUrl],
  ["Common", CommonnlUrl],
  ["Files", FilesnlUrl],
]);
const pl = new Map([
  ["Errors", ErrorsplUrl],
  ["Login", LoginplUrl],
  ["Common", CommonplUrl],
  ["Files", FilesplUrl],
]);
const pt = new Map([
  ["Errors", ErrorsptUrl],
  ["Login", LoginptUrl],
  ["Common", CommonptUrl],
  ["Files", FilesptUrl],
]);
const ptBR = new Map([
  ["Errors", ErrorsptBRUrl],
  ["Login", LoginptBRUrl],
  ["Common", CommonptBRUrl],
  ["Files", FilesptBRUrl],
]);
const ro = new Map([
  ["Errors", ErrorsroUrl],
  ["Login", LoginroUrl],
  ["Common", CommonroUrl],
  ["Files", FilesroUrl],
]);
const ru = new Map([
  ["Errors", ErrorsruUrl],
  ["Login", LoginruUrl],
  ["Common", CommonruUrl],
  ["Files", FilesruUrl],
]);
const si = new Map([
  ["Errors", ErrorssiUrl],
  ["Login", LoginsiUrl],
  ["Common", CommonsiUrl],
  ["Files", FilessiUrl],
]);
const sk = new Map([
  ["Errors", ErrorsskUrl],
  ["Login", LoginskUrl],
  ["Common", CommonskUrl],
  ["Files", FilesskUrl],
]);
const sl = new Map([
  ["Errors", ErrorsslUrl],
  ["Login", LoginslUrl],
  ["Common", CommonslUrl],
  ["Files", FilesslUrl],
]);
const srLatnRS = new Map([
  ["Errors", ErrorssrLatnRSUrl],
  ["Login", LoginsrLatnRSUrl],
  ["Common", CommonsrLatnRSUrl],
  ["Files", FilessrLatnRSUrl],
]);
const tr = new Map([
  ["Errors", ErrorstrUrl],
  ["Login", LogintrUrl],
  ["Common", CommontrUrl],
  ["Files", FilestrUrl],
]);
const ukUA = new Map([
  ["Errors", ErrorsukUAUrl],
  ["Login", LoginukUAUrl],
  ["Common", CommonukUAUrl],
  ["Files", FilesukUAUrl],
]);
const vi = new Map([
  ["Errors", ErrorsviUrl],
  ["Login", LoginviUrl],
  ["Common", CommonviUrl],
  ["Files", FilesviUrl],
]);
const zhCN = new Map([
  ["Errors", ErrorszhCNUrl],
  ["Login", LoginzhCNUrl],
  ["Common", CommonzhCNUrl],
  ["Files", FileszhCNUrl],
]);
export const translations = new Map([
  ["ar-SA", arSA],
  ["az", az],
  ["bg", bg],
  ["cs", cs],
  ["de", de],
  ["el-GR", elGR],
  ["en", en],
  ["es", es],
  ["fi", fi],
  ["fr", fr],
  ["hy-AM", hyAM],
  ["it", it],
  ["ja-JP", jaJP],
  ["ko-KR", koKR],
  ["lo-LA", loLA],
  ["lv", lv],
  ["nl", nl],
  ["pl", pl],
  ["pt", pt],
  ["pt-BR", ptBR],
  ["ro", ro],
  ["ru", ru],
  ["si", si],
  ["sk", sk],
  ["sl", sl],
  ["sr-Latn-RS", srLatnRS],
  ["tr", tr],
  ["uk-UA", ukUA],
  ["vi", vi],
  ["zh-CN", zhCN],
]);
