...
const { dbPath } = require('./src/config/config');
 ...
 */
async function main() {
  console.log(`${colors.bright}${colors.fg.cyan}Translation Key Usage - Setup${colors.reset}\n`);
  
  // Step 1: Check and install dependencies
  console.log(`${colors.fg.yellow}[1/3] Checking and installing dependencies...${colors.reset}`);
  try {
    ...
    console.error(`${colors.fg.red}✗ Error setting up dependencies:${colors.reset}`, error);
    process.exit(1);
  }
  
  // Step 2: Set up database directory
  console.log(`${colors.fg.yellow}[2/3] Setting up database directory...${colors.reset}`);
  try {
    ...
    console.log(`${colors.fg.green}✓ Database directory ready${colors.reset}\n`);
  } catch (error) {
    console.error(`${colors.fg.red}✗ Error setting up database directory:${colors.reset}`, error);
    process.exit(1);
  }
  
  // Step 3: Run initial analysis
  console.log(`${colors.fg.yellow}[3/3] Running initial codebase analysis...${colors.reset}`);
  try {
    ...
    console.error(`${colors.fg.red}✗ Error running analysis:${colors.reset}`, error);
    process.exit(1);
  }
  ...
  console.log(`2. Use the API endpoints at /api/key-usage/* to access key usage data`);
  console.log(`3. Integrate the UI components into your application`);
  console.log(`\n${colors.bright}View key usage in the UI:${colors.reset}`);
  console.log(`- Add the KeyUsageSearch component to your app for full key exploration`);
  console.log(`- Add TranslationKeyInfo to your translation editor for inline key information\n`);
}
