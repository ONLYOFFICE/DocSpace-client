...
const AiArbiter = ({ ensureAppsLoaded, fetchAppSettings }: AiArbiterProps) => {
  ...
  React.useEffect(() => {
    let cancelled = false;
    fetchAppSettings<AiArbiterSettings>("ai-arbiter")
      .catch(() => null)
      .then((settings) => {
        if (cancelled) return;
...
const AiArbiterConnected = inject<TStore>(({ appsStore }) => ({
...
export default AiArbiterConnected;
