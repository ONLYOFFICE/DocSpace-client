...
const Paging = (props: any) => {
  ...
  const onSelectPageAction = (option: any) => {
    props.onSelectPage(option);
  };

  const onSelectCountAction = (option: any) => {
  ...
  return (
    <StyledPaging id={id} className={className} style={style}>
      <Button
        // @ts-expect-error TS(2322): Type '{ className: string; size: string; scale: bo... Remove this comment to see the full error message
      ...
      {pageItems && (
        <StyledPage>
          <ComboBox
            // @ts-expect-error TS(2322): Type '{ dropDownMaxHeight: number; isDisabled: any... Remove this comment to see the full error message
      ...
      {showCountItem
        ? countItems && (
            <StyledOnPage>
              <ComboBox
                // @ts-expect-error TS(2322): Type '{ className: string; directionY: any; direct... Remove this comment to see the full error message
