...
const shouldSilenceWarnings = (...messages: any[]) =>
  [/Warning: componentWillReceiveProps has been renamed/].some((msgRegex) =>
    messages.some((msg) => msgRegex.test(msg))
  );

// @ts-expect-error TS(2339): Property 'app' does not exist on type 'Window & ty... Remove this comment to see the full error message
global.window.app = {
  mcApiUrl: "http://localhost:8080",
};

// setup file
const logOrThrow = (log: any, method: any, messages: any) => {
