...
describe("Ollama Translation Validation Tests", () => {
  test("Validate translations using Ollama LLM", async () => {
    ...
    for (const enFile of englishFiles) {
      ...
      const targetFiles = translationFiles.filter((file) => {
        return (
          ...
          (targetLanguages === null || targetLanguages.includes(file.language))
        );
      });
      ...
      for (const key of sampleKeys) {
        ...
        if (result && result.results) {
          ...
          if (result.summary.commonIssues && result.summary.commonIssues.length > 0) {
          ...
          if (invalidResults.length > 0) {
            ...
            invalidResults.slice(0, 3).forEach(invalid => {
              ...
              const targetText = keyTranslations.find(t => t.language === invalid.language)?.text || '';
    ...
    const languageStats = Object.entries(validationResults.errorsByLanguage)
      .map(([lang, errors]) => ({
        ...
        errorRate: Math.round((errors.length / validationResults.totalKeys) * 100)
      }))
  ...
  }, 0); // Disable timeout completely for this test
});
