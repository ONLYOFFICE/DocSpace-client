...
const Submenu = (props: any) => {
  ...
  const onCheckCurrentItem = useCallback(() => {
    const isSelect = data.find((item: any) => window.location.pathname.endsWith(item.id)
    );
  ...
  const selectSubmenuItem = (e: any) => {
    if (!forsedActiveItemId) {
      const item = data.find((el: any) => el.id === e.currentTarget.id);
