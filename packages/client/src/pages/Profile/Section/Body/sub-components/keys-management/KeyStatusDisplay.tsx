...
import type { KeyStatus } from "@docspace/shared/services/encryption/types";

import styles from "./keys-management.module.scss";

type KeyStatusDisplayProps = {
  status: KeyStatus;
};

export const KeyStatusDisplay: React.FC<KeyStatusDisplayProps> = ({
  status,
}) => {
    ...
    <div className={styles.keyStatus}>
      ...
      {status.createdAt && (
        <div className={styles.keyInfo}>
          ...
          <Text fontSize="13px">{status.createdAt.toLocaleDateString()}</Text>
        </div>
      )}
      <div className={styles.keyInfo}>
        <Text fontSize="13px" fontWeight={600}>
        ...
        <Text fontSize="13px">{status.algorithm}</Text>
      </div>
    </div>
  );
};
