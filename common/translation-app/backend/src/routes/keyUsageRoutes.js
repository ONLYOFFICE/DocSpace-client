        ...
        exec(grepCommand, (error, stdout) => {
          ...
          if (pendingSearches === 0) {
            ...
            const uniqueResults = results.filter((result, index, self) =>
              index === self.findIndex(r => r.file_path === result.file_path && r.line_number === result.line_number)
            );
...
async function keyUsageRoutes(fastify) {
  ...
  fastify.get("/api/key-usage/:key", async (request, reply) => {
    try {
      ...
      for (const file of metadataFiles) {
        ...
        if (file.data.comment_text !== undefined) {
          // Format comment data as expected by the frontend
          comment = {
            comment: file.data.comment_text || "",
            is_auto: file.data.comment_is_auto || false,
            updated_at: file.data.comment_updated_at || file.data.updated_at
          };
        }
  ...
  fastify.get("/api/keys-with-usage", async (request, reply) => {
    try {
      ...
      // Apply limit
      const limitedResults = limit ? keysWithUsage.slice(0, parseInt(limit)) : keysWithUsage;
  ...
  fastify.get("/api/key-usage/module/:module", async (request, reply) => {
    try {
            ...
            try {
              ...
              if (keyData.usage && keyData.usage.length > 0) {
                const usageInModule = keyData.usage.some(u => {
                  return u.module === module || 
                         (u.file_path && u.file_path.includes(`/packages/${module}/`));
                });
      ...
      return reply.send({
        module,
        keys: keysForModule.map(k => k.namespace && k.namespace !== 'Common' ? `${k.namespace}:${k.key}` : k.key)
      });
    } catch (error) {
      request.log.error(error);
              ...
              if (keyData.usage && keyData.usage.length > 0) {
                for (const usage of keyData.usage) {
                  if (usage.module) {
                    ...
                    const moduleMatch = usage.file_path.match(/\/packages\/([^\/]+)\//); 
                    if (moduleMatch) {
    ...
    try {
      ...
      for (const file of metadataFiles) {
        try {
          ...
          console.error(`Error updating metadata file ${file.metaPath}:`, error);
        }
      }
    ...
    try {
      ...
      for (const file of metadataFiles) {
        try {
          ...
          console.error(`Error updating comment in metadata file ${file.metaPath}:`, error);
        }
      }
      
      if (updatedFiles.length === 0) {
        return reply.code(500).send({ error: "Failed to update any metadata files with comment" });
      }
  ...
  fastify.post("/api/key-usage/analyze", async (request, reply) => {
    try {
      ...
      const analyzeAllKeys = async () => {
        ...
        for (const file of allMetadataFiles) {
          try {
            ...
            if (processedCount % 50 === 0) {
              console.log(`Analyzed ${processedCount}/${allMetadataFiles.length} keys (${updatedCount} updated)`);
            }
          } catch (error) {
        ...
        console.log(`Analysis complete. Processed ${processedCount} keys, updated ${updatedCount} metadata files.`);
      };
  ...
  fastify.get("/api/key-usage/stats", async (request, reply) => {
    try {
      ...
      for (const module of totalModules) {
        const keyCount = allMetadataFiles.filter(file => 
          file.data && file.data.usage && 
          file.data.usage.some(u => u.module === module)
        ).length;
        
        const fileCount = allMetadataFiles.filter(file => 
          file.data && file.data.usage && 
          file.data.usage.some(u => u.file_path && u.file_path.includes(`/packages/${module}/`))
        ).length;
    ...
    try {
      ...
      for (const file of allMetadataFiles) {
        if (!file.data || !file.data.usage || file.data.usage.length === 0) {
          const fullKey = file.namespace && file.namespace !== 'Common' ? 
                          `${file.namespace}:${file.key}` : file.key;
      ...
      request.log.error(error);
      return reply.code(500).send({ error: "Failed to get missing usage keys" });
    }
  });
};
