import { useState, useLayoutEffect, createElement } from "react";
...
const ChatView = ({ items, currentDeviceType }: ChatTypes) => {
  ...
  return <div style={{ height: `${height}px` }}>{chat}</div>;
};

export default inject(({ flowStore, settingsStore }: TStore) => {
  const { vectorizedFiles } = flowStore;
