...
import PrivateArchivePage from "./page.client";
...
export default async function PrivateArchive({
  ...
  const filter = getPrivateRoomsDefaultFilter(RoomSearchArea.Archive);
  ...
  try {
    ...
    throw new Error(
      `Failed to load private archive page data: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
