...
// @ts-expect-error TS(2307): Cannot find module 'PUBLIC_DIR/images/avatar.base.... Remove this comment to see the full error message
import AvatarBaseReactSvgUrl from "PUBLIC_DIR/images/avatar.base.react.svg?url";
// @ts-expect-error TS(2307): Cannot find module 'PUBLIC_DIR/images/remove.sessi... Remove this comment to see the full error message
...
const FillingRoleSelector = ({
  ...
  const sortedInOrderRoles = cloneRoles.sort((a: any, b: any) =>
    a.order > b.order ? 1 : -1
  );

  const everyoneRole = roles.find((item: any) => item.everyone);
  ...
  return (
    <StyledFillingRoleSelector {...props}>
      ...
      {sortedInOrderRoles.map((role: any, index: any) => {
        if (role.everyone) return;
        const roleWithUser = users?.find((user: any) => user.role === role.name);
