const { initializeDatabase, getKeyRecord } = require("../utils/dbUtils");
...
async function keyUsageRoutes(fastify) {
  ...
  fastify.get("/api/key-usage/:key", async (request, reply) => {
    try {
      ...
      console.error(
        `Error getting key usage for ${request.params.key}:`,
        error
      );
      return reply
        .code(500)
        .send({ error: "Failed to get key usage information" });
    }
  });
  ...
  fastify.get("/api/key-usage/module/:module", async (request, reply) => {
    try {
      ...
      console.error(
        `Error getting keys for module ${request.params.module}:`,
        error
      );
  ...
  fastify.post("/api/key-usage/:key/comment", async (request, reply) => {
    try {
      ...
      if (!keyRecord) {
        ...
        keyRecord = await getKeyRecord(db, key);
      }
      ...
      console.error(
        `Error setting comment for key ${request.params.key}:`,
        error
      );
  ...
  fastify.get("/api/key-usage/search", async (request, reply) => {
    try {
      ...
      console.error(
        `Error searching for keys with query ${request.query.query}:`,
        error
      );
  ...
  fastify.get("/api/key-usage/stats", async (request, reply) => {
    try {
      ...
      return reply
        .code(500)
        .send({ error: "Failed to get key usage statistics" });
    }
  });
}
