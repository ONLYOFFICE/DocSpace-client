...
The last three tolerate inherited debt through
`common/tests/test/locale-punctuation-allowlist.json`. Like the license
allowlists it **must only ever shrink** — fix the string and drop its entry in
the same commit; never add a new one to make a check pass.
