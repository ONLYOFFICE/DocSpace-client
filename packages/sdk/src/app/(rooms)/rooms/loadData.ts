...
export async function loadRoomsPageData(
  ...
  const [filesSettings, roomsData, portalSettings, user] = await Promise.all([
    deps.getFilesSettings(),
    deps.getRooms(filter),
    deps.getSettings(),
    deps.getSelf(),
  ]);

  if (!filesSettings || !portalSettings || !roomsData) return null;
