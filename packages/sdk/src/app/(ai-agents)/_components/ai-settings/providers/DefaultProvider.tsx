...
const DefaultProviderComponent = ({ aiConfig }: DefaultProviderProps) => {
  ...
  // ServicesStore.formatAiModelsCurrency has no SDK equivalent — fall back
    ...
    return (
      aiProviders?.map((p) => {
        ...
        return {
          ...
          // SDK is iframe-bound and cannot navigate there directly. Omit the
          // click handler — the external-link affordance still surfaces the
          // path to the host.
    ...
    return (
      <div style={{ display: "contents" }}>
        {defaultProviderModels.map((m) => {
          ...
          const safeFormat = (v: number) => String(v);
          const priceLabel =
            m.price != null
...
export const DefaultProvider = observer(DefaultProviderComponent);
