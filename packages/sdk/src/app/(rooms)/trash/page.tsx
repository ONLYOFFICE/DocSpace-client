...
export default async function Trash({
  ...
  try {
    [filesSettings, portalSettings, user, tree] = await Promise.all([
      getFilesSettings(),
      getSettings(),
      getSelf(),
      getFoldersTree(),
    ...
    filter.folder = params.folder || String(trashFolderId);

    folderData = await getFolder(filter.folder, filter);
  } catch (error) {
    throw new Error(
      `Failed to load trash page data: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
