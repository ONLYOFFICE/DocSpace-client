...
const ErrorContainer = (props: ErrorContainerProps) => {
  ...
  const buttonId = withImg ? "button-container-with-image" : "button-container";
  const buttonSize = withImg ? ButtonSize.small : ButtonSize.normal;
  const headerId = withImg ? "header-with-image" : "header";
  const textId = withImg ? "customized-text-with-image" : "customized-text";

  return (
    <StyledErrorContainer {...rest} data-testid="ErrorContainer">
      <div id="container">
        {withImg ? (
          <img src={img} alt="error-img" id="error-img" />
        ) : (
          <>
            <svg
              id="background"
      ...
      {headerText && (
        <Headline id={headerId} type="header">
          {headerText}
        </Headline>
      )}
      {bodyText && <Text id="text">{bodyText}</Text>}
      {customizedBodyText && <Text id={textId}>{customizedBodyText}</Text>}
