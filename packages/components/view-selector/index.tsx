...
const ViewSelector = ({
  ...
  const onChangeViewHandler = (e: any) => {
    ...
    if (view !== viewAs) {
      const option = viewSettings.find((setting: any) => view === setting.value);
  ...
  const renderFewIconView = () => {
    return viewSettings.map((el: any, indx: any) => {
  ...
  const renderOneIconView = () => {
    const element = viewSettings.find((el: any) => el.value != viewAs);
